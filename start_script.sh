...
if ! conda env list | grep -qE "^$CONDA_ENV_NAME\s"; then
    ...
    echo "Installing requirements from dashboard/requirements.txt..."
    pip install -r "$BASE_DIR/dashboard/requirements.txt" # Corrected path from dasboard to dashboard
    conda deactivate
